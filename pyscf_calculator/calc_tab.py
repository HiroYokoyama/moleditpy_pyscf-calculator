...
class CalcTab(QWidget):
    ...
    def setup_ui(self):
        ...
        self.functional_combo.setEnabled(True)
        ...
        self.check_symmetry.setToolTip(
            "Detect and use point group symmetry to speed up calculation"
        )
        ...
        self.check_break_sym.setToolTip(
            "For UKS/UHF: Mix Alpha/Beta densities in initial guess to encourage spin polarization."
        )
        ...
        self.spin_grid_level.setToolTip(
            "DFT Integration Grid Level (Default: 3). Set to 4 or 5 for higher accuracy in optimizations."
        )
        ...
        self.out_dir_edit.setToolTip(
            "Path to save results. Relative paths (e.g. 'results') will be resolved relative to the current file location or default to the home directory."
        )
        ...
        self.btn_save_default.setToolTip(
            "Save current settings (Job, Method, Path, etc.) as default for new projects."
        )
    ...
    def update_options(self, text=None):
        ...
        if "Scan" in job:
            if getattr(self, "btn_scan_config", None) is not None:
                self.btn_scan_config.show()
        else:
            if getattr(self, "btn_scan_config", None) is not None:
                self.btn_scan_config.hide()
        ...
        job = self.job_type_combo.currentText()
        if getattr(self, "lbl_nstates", None) is not None and hasattr(
            self, "nstates_input"
        ):
            is_tddft = job == "TDDFT"
    ...
    def auto_detect_charge_spin(self):
        ...
        try:
            ...
            if idx_c >= 0:
                self.charge_input.setCurrentIndex(idx_c)
            else:
                self.charge_input.setCurrentText(str(charge))
            ...
            if suggested_spin == 1:
                if current_method == "UHF":
                    new_method = "RHF"
                elif current_method == "UKS":
                    new_method = "RKS"
            else:
                if current_method == "RHF":
                    new_method = "UHF"
                elif current_method == "RKS":
                    new_method = "UKS"
            ...
            if has_transition_metal:
                self.log(
                    "Auto-Detect: Transition metal detected. High spin states may be possible."
                )
    ...
    def configure_scan(self):
        ...
        if getattr(self, "scan_params", None) is None:
            self.scan_params = None
        ...
        if not os.path.isabs(raw_out_dir):
            ...
            if not current_path:
                ...
                fallback_path = os.path.abspath(
                    os.path.join(fallback_base, raw_out_dir)
                )

                msg = (
                    f"You are using a relative path ('{raw_out_dir}') with an unsaved project.\n"
        ...
        if self.context:
            ...
            if mw:
                ...
                if splitter:
                    ...
                    QTimer.singleShot(
                        0, lambda: splitter.setSizes([0, splitter.width()])
                    )
