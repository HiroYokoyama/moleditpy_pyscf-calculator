...
class VisTab(QWidget):
    ...
    def _add_to_history(self, d):
        if not hasattr(self.parent_dialog, "calc_history"):
            self.parent_dialog.calc_history = []
        ...
        if not os.path.exists(chk_path):
            ...
            if os.path.exists(chk_path_alt):
                ...
                QMessageBox.warning(
                    self,
                    "Error",
                    f"No checkpoint file (pyscf.chk) or scan results found in {d}",
                )
                return
    ...
    def on_load_finished(self, result_data):
        ...
        if result_data.get("loaded_xyz", None):
            ...
            self.parent_dialog.optimized_xyz = result_data[
                "loaded_xyz"
            ]  # Expose to parent
        ...
        if should_update_geom and self.optimized_xyz:
            ...
            def update_and_finalize():
                ...
                mw = self.context.get_main_window()
                uim = getattr(mw, "ui_manager", None)
        ...
        if result_data.get("freq_data", None):
            ...
            try:
                ...
                if mol:
                    ...
                    self.freq_dock.setAllowedAreas(
                        Qt.DockWidgetArea.RightDockWidgetArea
                    )
                    mw.addDockWidget(
                        Qt.DockWidgetArea.RightDockWidgetArea, self.freq_dock
                    )
                    self.freq_dock.show()
    ...
    def run_specific_analysis(self, tasks, out_d=None):
        ...
        if not out_d:
            ...
            if not out_d and self.chkfile_path:
                out_d = os.path.dirname(self.chkfile_path)
            if not out_d:
                out_d = self.parent_dialog.out_dir_edit.text()
    ...
    def on_prop_results(self, result_data):
        ...
        self.file_list.clear()
        d = self.last_out_dir
        if not d and self.chkfile_path:
    ...
    def update_visualization(self):
        if self.mode == "mapped":
            self.update_mapped_vis()
            return
        if not self.visualizer or not self.loaded_file:
            return
    ...
    def fit_mapped_range(self):
        ...
        if p_min < cur_min:
            self.m_min_spin.setMinimum(p_min - 1.0)
        if p_max > cur_max:
            self.m_max_spin.setMaximum(p_max + 1.0)
        self.m_min_spin.setValue(p_min)
        self.m_max_spin.setValue(p_max)
        self.update_mapped_vis()
    ...
    def load_optimized_geometry(self):
        if self.optimized_xyz:
            ...
            QMessageBox.information(
                self, "Success", "Geometry updated with optimized structure."
            )
    ...
    def add_custom_mo(self):
        ...
        if not (is_digit or is_rel):
            QMessageBox.warning(
                self,
                "Invalid Input",
                "Please enter a valid integer index or relative label.",
            )
            return
    ...
    def show_energy_diagram(self):
        ...
        self.energy_dlg = EnergyDiagramDialog(
            self.mo_data, parent=self, result_dir=self.last_out_dir
        )
    ...
    def export_thermo_csv(self, table):
        fname, _ = QFileDialog.getSaveFileName(
            self, "Export CSV", "thermo.csv", "CSV (*.csv)"
        )
        if not fname:
            return
        try:
