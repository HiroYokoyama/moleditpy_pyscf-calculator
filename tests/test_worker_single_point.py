...
class TestSinglePointCompletion(unittest.TestCase):
    ...
    def test_input_script_omits_the_xyz_header(self):
        ...
        _mod.gto.M.return_value = _make_mock_mol()
        _mod.dft = MagicMock()
        _mod.dft.RKS.return_value = _make_mock_mf()
        w = _make_worker(
            {
                "job_type": "Energy",
                "method": "RKS",
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch.object(_mod, "CaptureStdOut") as mock_cap:
                ...
                mock_cap.return_value.__exit__ = MagicMock(return_value=False)
                w.run()
            path = os.path.join(tmpdir, "job_1", "pyscf_input.py")
            with open(path, encoding="utf-8") as fh:
                script = fh.read()
        compile(script, path, "exec")
        self.assertIn("atom='''H 0 0 0\nH 0 0 0.74'''", script)
        self.assertNotIn("Generated by PySCF Plugin", script)
        self.assertIn("max_memory=2500", script)
        self.assertIn("mf.grids.level = 5", script)
        # only what the script uses (scf was imported for DFT jobs too)
        self.assertIn("from pyscf import gto, dft", script)
        self.assertNotIn("scf", script.split("mol = gto.M")[0].split("import")[-1])
